...
__global__ void min_lstm_extract_scan_params_kernel(const LinearLayer* d_linear_f,
                                                   const LinearLayer* d_linear_i,
    ...
    if (t < seq_length && h < hidden_size) {
        ...
        a[t * hidden_size + h] = f_prime;
        b[t * hidden_size + h] = i_prime * h_tilde;
    }
}
...
void min_lstm_parallel_scan_cuda(int seq_length, int batch_size, int hidden_size,
...
void min_lstm_to_device(const MinLSTMCell* cell, LinearLayer* d_linear_f, LinearLayer* d_linear_i, 
    ...
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_linear_f->weights, 
                                cell->hidden_size * cell->input_size * sizeof(float)));
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_linear_f->bias, 
                                cell->hidden_size * sizeof(float)));
    
    CHECK_CUDA_ERROR(cudaMemcpy(d_linear_f->weights, cell->linear_f.weights, 
                                cell->hidden_size * cell->input_size * sizeof(float), 
                                cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(d_linear_f->bias, cell->linear_f.bias, 
                                cell->hidden_size * sizeof(float), 
                                cudaMemcpyHostToDevice));
    ...
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_linear_i->weights, 
                                cell->hidden_size * cell->input_size * sizeof(float)));
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_linear_i->bias, 
                                cell->hidden_size * sizeof(float)));
    
    CHECK_CUDA_ERROR(cudaMemcpy(d_linear_i->weights, cell->linear_i.weights, 
                                cell->hidden_size * cell->input_size * sizeof(float), 
                                cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(d_linear_i->bias, cell->linear_i.bias, 
                                cell->hidden_size * sizeof(float), 
                                cudaMemcpyHostToDevice));
...
void min_lstm_free_device(LinearLayer* d_linear_f, LinearLayer* d_linear_i, LinearLayer* d_linear_h) {
    cudaFree(d_linear_f->weights);
    cudaFree(d_linear_f->bias);
    cudaFree(d_linear_i->weights);
    cudaFree(d_linear_i->bias);
    cudaFree(d_linear_h->weights);
    cudaFree(d_linear_h->bias);
}

void min_lstm_process_sequence_cuda(const MinLSTMCell* cell, const float* x, const float* h0,
    ...
    min_lstm_extract_scan_params_kernel<<<extractGridDim, extractBlockDim>>>(
        &d_linear_f, &d_linear_i, &d_linear_h, d_x, d_a, d_b, 
        seq_length, cell->hidden_size, cell->input_size
    );
    
    min_lstm_parallel_scan_cuda(seq_length, 1, cell->hidden_size, d_a, d_b, d_h0, d_h_out);
